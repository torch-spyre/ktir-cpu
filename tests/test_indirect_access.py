...
"""End-to-end tests for ktdp.construct_indirect_access_tile.

Full stack: MLIR parsing → IAT construction → indirect_load/store → HBM
verification.  Complements test_blocked_indirect_fast_path.py which tests
the numpy-level fast path in isolation (without MLIR parsing).
"""
...
# ---------------------------------------------------------------------------
# Helpers: load MLIR, seed HBM with test data, run a function.
# All tests follow the same pattern: seed → execute → read result from HBM.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# MLIR templates: each generates a complete MLIR module for a specific IAT
# pattern.  Parameterised on indirect_side ("load" or "store") to test both
# gather and scatter using the same index layout.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RFC example: exercises the canonical indirect-access-copy.mlir from the spec.
# Smoke test only (zero-seeded) — verifies parse + execution don't crash.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SSA-as-intermediate-variable validation: an outer SSA value (e.g. %c2 = 2)
# can appear as an intermediate variable only if its dimension's range is
# [0, 0] (pinned — the iterator never moves).  If the range is wider, the
# iterator would silently overwrite the SSA value at each step → wrong results.
# The interpreter must reject this at construction time.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Scatter (indirect store): Y[IDX1[m,k], IDX2[m,k]] = X[m,k]
# The IAT is on the destination side; the source is a regular tile.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Negative-index guard: index arrays must not contain negative values.
# Unlike numpy (which wraps), negative indices in hardware would be UB.
# The interpreter raises IndexError at runtime.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Non-identity variables_space_order (VSO)
#
# VSO controls the iteration order over the variables space.  The default
# (identity) iterates row-major.  A swap (d0,d1)->(d1,d0) iterates
# column-first.  The result shape stays the same, but data lands at
# transposed positions — the output of a gather with swapped VSO is X^T.
#
# RFC 0682 §473: vso defines lexicographic traversal order (rightmost output
# dimension = innermost iteration loop).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3-cycle VSO (non-involution): (d0,d1,d2)->(d2,d0,d1)
#
# A swap is an involution (applying twice = identity), so any implementation
# that applies vso as a coordinate transform instead of using it as a sort
# key would still pass the swap test by accident.  The 3-cycle is NOT an
# involution — applying it as a transform gives a DIFFERENT result than
# sorting by it.  This test distinguishes the spec-correct implementation
# (sort-then-access) from the common wrong implementation (transform-then-access).
#
# Based on RFC 0682 §473 worked example (issue #58 discussion).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Subscript expression parser/evaluator: these are the helpers that convert
# MLIR subscript text ("%d0 floordiv 64") into AST tuples and evaluate them
# at a given iteration point.  Tested in isolation (no MLIR module needed).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Integration: compound floordiv/mod in direct subscripts.
#
# Real workloads often compute a flat linear index and then decompose it:
#   dst_row = (page * 64 + lane) floordiv 64
#   dst_col = (page * 64 + lane) mod 64
#
# This exercises the MLIR parser path for compound-LHS floordiv/mod, which
# the unit tests above bypass (they call parse_subscript_expr directly).
# ---------------------------------------------------------------------------
